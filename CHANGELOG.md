...
### Added
- Workflow linting in CI: an `actionlint` job in `ci.yml`, gated on a new `workflows` path filter (`.github/workflows/**`) and registered in the `status` sentinel that branch protection keys on. Installed from the pinned `v1.7.12` release tarball verified against a hard-coded sha256 rather than the upstream install script, so the gate is reproducible and the download is checked. It runs with `-shellcheck=`, which disables the shellcheck pass over inline `run:` blocks: the runner ships shellcheck, so that pass is deterministic here and is off only because 59 pre-existing SC2086/SC2129 findings are unfixed. The flag comes off with them (#255). Kept out of pre-commit deliberately, since a hook fires only for contributors who ran `pre-commit install` and is skippable with `--no-verify` (#215)
