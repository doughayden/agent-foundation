...
- `docs/references/agent-evals.md`: the complete agent-evaluation map — data formats, every run path (`uv run server` dev UI, `uv run pytest tests/eval`, `adk eval`/`adk test`/`adk eval_set`/`adk migrate`), a metrics table, the deterministic CI gate, and dynamic user simulation — with ADK's docs as the source of truth for mechanics (#104)
