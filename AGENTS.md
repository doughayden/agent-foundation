...
**Pre-commit:** Local fix-in-place hooks in `.pre-commit-config.yaml` cover Python (ruff, mypy), Terraform (`terraform fmt`), and config-file syntax. `pre-commit autoupdate` is a no-op for the `language: system` hooks — ruff/mypy come from `uv.lock` (`uv lock --upgrade`), terraform from the local binary (system package manager); autoupdate DOES bump the pinned `rev` of upstream-repo hooks. Note CI's `terraform fmt -check` covers `terraform/main` ONLY (`working-directory` in `terraform-plan-apply.yml`) — the hook is the only fmt enforcement for `terraform/bootstrap/**`. **Two checks are deliberately NOT hooks:** `terraform validate` (needs `terraform init` per root — too heavy for a commit; CI validates) and workflow linting (see below).

**Workflow lint (Template Internals):** `actionlint` job in `ci.yml`, gated on the `changes` job's `workflows` filter (`.github/workflows/**`) and registered in the `status` sentinel. **Deliberately CI-only, NOT pre-commit** (#215): a hook fires only for contributors who ran `pre-commit install` and is skippable with `--no-verify`, so the files defining the pipeline get a merge gate instead. Installed from a pinned release tarball verified against a hard-coded sha256, not the upstream install script — bump `ACTIONLINT_VERSION` and `ACTIONLINT_SHA256` together (checksums published per release as `actionlint_<ver>_checksums.txt`). **Runs with `-shellcheck=`**, which disables the shellcheck pass over inline `run:` blocks; the runner ships shellcheck so the pass would be deterministic here, and it is off ONLY because 59 pre-existing SC2086/SC2129 findings are unfixed (#255). Remove the flag with those findings. A related but separate surface: `zizmor` (security audits — template injection, unpinned actions, excessive permissions) is pip-installable and would pin in `uv.lock`, but reports ~200 findings repo-wide; tracked separately, do NOT bolt it onto the actionlint job.
