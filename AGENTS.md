...
**Pre-commit:** Local fix-in-place hooks are configured in `.pre-commit-config.yaml`, covering Python (ruff, mypy), Terraform (`terraform fmt`), workflows (actionlint), and config-file syntax. `pre-commit autoupdate` is a no-op for the `language: system` hooks (ruff/mypy pinned in `uv.lock`, terraform from the local binary) — use `uv lock --upgrade`; it DOES bump the `rev` of upstream-repo hooks like actionlint. **`terraform validate` is deliberately NOT a hook** (requires `terraform init` per root — too heavy for a commit; CI validates). **actionlint runs with `-shellcheck=`**: actionlint shells out to shellcheck for inline `run:` scripts only when that binary is on PATH, so leaving it enabled makes the finding set machine-dependent; the 59 existing SC2086/SC2129 findings are tracked separately (#255) and the flag comes off when they land. Note CI's `terraform fmt -check` covers `terraform/main` ONLY (`working-directory` in `terraform-plan-apply.yml`) — the hook is the only fmt enforcement for `terraform/bootstrap/**`.
