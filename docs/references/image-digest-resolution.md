...
2. **The tag digest never matches the revision digest.** Cloud Run pins every revision to a digest at deploy time ([deploying images](https://docs.cloud.google.com/run/docs/deploying): "If you deploy from a container image tag, it will be resolved to a digest and the revision will always serve this particular digest"). When the deployed digest is an index, the revision records the resolved `linux/amd64` platform manifest digest — observed behavior in this project (verified by inspecting the index children), not documented upstream. A mismatch between tag digest and revision digest is expected, not a deployment integrity problem.
