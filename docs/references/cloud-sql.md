...
- **Idempotent:** COS re-runs cloud-init on every boot; `CREATE EXTENSION IF NOT EXISTS` is a no-op and the named job upserts, so reboots re-assert exactly one job.
